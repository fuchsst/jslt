 ...
 */
class FailDotExpression(location: Location?, private val where: String) : DotExpression(location = location) {
    // verify that we've build a correct DotExpression for our object
    // matcher (only used for that)
    override fun checkOk(matcher: Location?) {
        // we're actually being used. this is illegal!
        throw JsltException("Object matcher used inside $where", matcher)
    }
}